...
def draw_sculpt_ui(self, context, layout):
    ...
    if obj != None and obj.mode == "SCULPT":
        ...
        subrow.prop(obj.coa_tools2, "selected_shapekey", text="")
        op = subrow.operator("coa_tools2.shapekey_add", icon="FILE_NEW", text="")

        if obj.data.shape_keys != None and len(obj.data.shape_keys.key_blocks) > 0:
            op = subrow.operator(
                "coa_tools2.shapekey_rename", icon="OUTLINER_DATA_FONT", text=""
            )
            op = subrow.operator("coa_tools2.shapekey_remove", icon="X", text="")

        if obj.data.shape_keys != None:
            shapekey_index = int(obj.coa_tools2.selected_shapekey)
            if shapekey_index > 0:
    ...
    if context.area != None:
        if context.area.type == "VIEW_3D":
            ...
            view_3d_region_x = Vector(
                (
                    context.area.x + t_panel.width,
                    context.area.x + context.area.width - n_panel.width,
                )
            )
            ...
            if (
                mouse_pos.x > view_3d_region_x[0]
                and mouse_pos.x < view_3d_region_x[1]
                and mouse_pos.y > view_3d_region_y[0]
                and mouse_pos.y < view_3d_region_y[1]
            ):
                in_view_3d = True
            else:
                in_view_3d = False
...
def unwrap_with_bounds(obj, uv_idx):
    ...
    offset = [
        get_local_dimension(obj)[2][0] * scale_x,
        get_local_dimension(obj)[2][1] * scale_z,
    ]
...
def create_action(context, item=None, obj=None):
    ...
    if len(sprite_object.coa_tools2.anim_collections) < 3:
...
def clear_pose(obj):
    if obj.type == "ARMATURE":
        ...
        obj.coa_tools2.alpha = 1.0
        obj.coa_tools2.modulate_color = (1.0, 1.0, 1.0)
        obj.coa_tools2.slot_index = 0
...
def set_action(context, item=None):
    ...
    for child in animation_objects:
        ...
        if child.type == "MESH" and item.name == "Restpose":
            child.coa_tools2.alpha = 1.0
            child.coa_tools2.modulate_color = (1.0, 1.0, 1.0)
        elif (
...
def actions_callback(self, context):
    ...
    actions.append(
        ("Clear Action", "Clear Action", "Clear Action", "RESTRICT_SELECT_ON", 1)
    )
    i = 2
...
def ray_cast(start, end, list=[]):
    ...
    if b_version_smaller_than((2, 91, 0)):
        ...
        result = bpy.context.scene.ray_cast(
            bpy.context.view_layer.depsgraph, start, end
        )
...
def display_children(self, context, obj):
    ...
    if context.scene.coa_tools2.active_collection in bpy.data.collections:
    ...
    if obj != None:
        ...
        col = layout.column(align=True)
        row = col.row(align=True)
        if context.scene.coa_tools2.display_all:
            row.prop(
                context.scene.coa_tools2,
                "display_all",
                text="",
                toggle=True,
            ...
            row.prop(context.scene.coa_tools2, "display_length", text="Length")
            row.prop(context.scene.coa_tools2, "display_page", text="Page")
        row.prop(context.scene.coa_tools2, "active_collection", text="")
        box = col.box()
        ...
        row.prop(obj.coa_tools2, "filter_names", text="", icon="VIEWZOOM")
        if sprite_object != None:
            if sprite_object.coa_tools2.favorite:
                row.prop(sprite_object.coa_tools2, "favorite", text="", icon="SOLO_ON")
            else:
                row.prop(sprite_object.coa_tools2, "favorite", text="", icon="SOLO_OFF")
        ...
        for obj2 in context.view_layer.objects:
            if get_sprite_object(obj2) == obj2:
                in_range = current_display_item in range(
                    context.scene.coa_tools2.display_page
                    * context.scene.coa_tools2.display_length,
                    context.scene.coa_tools2.display_page
                    * context.scene.coa_tools2.display_length
                    + context.scene.coa_tools2.display_length,
                )

                if in_range or context.scene.coa_tools2.display_all:
                    row = col.row(align=True)
                    ...
                    if get_sprite_object(obj2) == sprite_object:
                        if (
                            obj2.coa_tools2.show_children
                            and get_sprite_object(obj2) == sprite_object
                        ):
                            subrow.prop(
                                obj2.coa_tools2,
                                "show_children",
                    ...
                    op = subrow.operator(
                        "coa_tools2.select_child", text=obj2.name, emboss=False
                    )
                    op.mode = "object"
                    ...
                    if armature.mode == "OBJECT":
                        ...
                        icon = "EDITMODE_HLT"
                    op = subrow.operator(
                        "coa_tools2.toggle_pose_mode", icon=icon, text="", emboss=False
                    )
                    op.ob_name = armature.name

                    op = subrow.operator(
                        "coa_tools2.view_sprite",
                        icon="ZOOM_SELECTED",
                        text="",
                        emboss=False,
                    )
                    ...
                    row2 = subrow.row()
                    if not obj2.coa_tools2.change_z_ordering:
                        row2.active = False
                    row2.prop(
                        obj2.coa_tools2,
                        "change_z_ordering",
...
def draw_children(
    ...
    if sprite_object != None and sprite_object.coa_tools2.show_children:
        children = sorted(
            children,
            key=lambda x: x.location[1] if type(x) == bpy.types.Object else x.name,
            reverse=False,
        )
        children = sorted(
            children,
            key=lambda x: x.type if type(x) == bpy.types.Object else x.name,
            reverse=False,
        )
        if (
            len(children) > 1
            and type(children[1]) == bpy.types.Object
            and children[1].type == "CAMERA"
        ):
        ...
        for i, child in enumerate(children):
            if child.name in active_collection.objects:
                ...
                name_found = (
                    obj.coa_tools2.filter_names.upper() in child.name.upper()
                    and not obj.coa_tools2.filter_names == ""
                )
                current_display_item += 1
                if in_range or context.scene.coa_tools2.display_all or name_found:
                    if (
                        ...
                        if obj.coa_tools2.filter_names.upper() in child.name.upper():
                            row = col.row(align=True)
                            ...
                            if (
                                child.type == "MESH"
                                and len(child.data.vertices) > 4
                                and child.data.coa_tools2.hide_base_sprite == False
                                and "coa_base_sprite" in child.vertex_groups
                                and "coa_base_sprite" in child.modifiers
                            ):
                                subrow2.prop(
                                    child.data.coa_tools2,
                                    "hide_base_sprite",
                                    text="",
                                    icon="ERROR",
                                    emboss=False,
                            ...
                            op = subrow2.operator(
                                "coa_tools2.select_child", text=name, emboss=False
                            )
                            op.mode = "object"
                            op.ob_name = child.name

                            # if child.type == "ARMATURE":
                            #     if child.coa_tools2.show_bones:
                            #         subrow2.prop(child.coa_tools2,"show_bones",text="",icon="TRIA_DOWN",emboss=False)
                            #     else:
                            #         subrow2.prop(child.coa_tools2,"show_bones",text="",icon="TRIA_LEFT",emboss=False)

                            if child.type == "MESH" and child.coa_tools2.type == "SLOT":
                                if child.coa_tools2.slot_show:
                                    subrow2.prop(
                                        child.coa_tools2,
                                        "slot_show",
                                        text="",
                            ...
                            if not sprite_object.coa_tools2.change_z_ordering:
                                ...
                                if child.coa_tools2.hide_select:
                                    subrow2.prop(
                                        child.coa_tools2,
                                        "hide_select",
                                        emboss=False,
                            ...
                            if (
                                child.type == "MESH"
                                and child.coa_tools2.type == "SLOT"
                                and child.coa_tools2.slot_show
                            ):
                                for i, slot in enumerate(child.coa_tools2.slot):
                                    row = col.row()
                                    subrow = row.row(align=True)
                            ...
                            if (
                                child.type == "MESH"
                                and sprite_object.coa_tools2.edit_armature
                            ):
                                op = subrow3.operator(
                                    "coa_tools2.bind_mesh_to_bones",
                                    text="",
                            ...
                            # if child.type == "ARMATURE":
                            #     if (not sprite_object.coa_tools2.favorite and child.coa_tools2.show_bones) or sprite_object.coa_tools2.favorite:
                            #         for bone in child.data.bones:
                            #             if (sprite_object.coa_tools2.favorite and bone.coa_tools2.favorite or not sprite_object.coa_tools2.favorite):
...
def draw_bone_entry(self, bone, row, col, child, indentation_level=0):
    ...
    bone_name = "" + bone.name
    op = row.operator("coa_tools2.show_children", text=bone_name, emboss=False)
    op.mode = "bone"
    op.ob_name = child.name
    op.bone_name = bone.name
    if bone.coa_tools2.favorite:
        row.prop(bone.coa_tools2, "favorite", emboss=False, text="", icon="SOLO_ON")
    else:
        row.prop(bone.coa_tools2, "favorite", emboss=False, text="", icon="SOLO_OFF")
    if bone.hide:
        row.prop(bone.coa_tools2, "hide", text="", emboss=False, icon="HIDE_ON")
    else:
        row.prop(bone.coa_tools2, "hide", text="", emboss=False, icon="HIDE_OFF")
    if bone.hide_select:
        row.prop(
            bone.coa_tools2,
            "hide_select",
