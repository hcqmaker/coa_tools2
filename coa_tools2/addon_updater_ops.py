...
class addon_updater_install_popup(bpy.types.Operator):
    ...
    def draw(self, context):
        ...
        if updater.update_ready == True:
            ...
            col.scale_y = 0.7
            col.label(
                text="Update ready! Press OK to install " + str(updater.update_version),
                icon="LOOP_FORWARDS",
            )
...
def update_notice_box_ui(self, context):
    ...
    if updater.manual_only == False:
        ...
        col.operator(
            addon_updater_update_now.bl_idname, text="Update now", icon="LOOP_FORWARDS"
        )
...
def register(bl_info):
    ...
    updater.include_branch_list = ["master"]  # is the equivalent to setting ['master']
