...
class COAOutliner(bpy.types.PropertyGroup):
    ...
    entry_type: StringProperty(
        default="SPRITE"
    )  # ["SPRITE", "OBJECT","SLOT", "BONE", "BONE_PARENT"]
    hierarchy_level: IntProperty()
    slot_type: StringProperty()
    selected: BoolProperty(
        default=False, update=selected_update
...
class COATOOLS2_UL_Outliner(bpy.types.UIList):
    def draw_item(
        ...
        if item.entry_type == "UPDATE_SPRITE_OBJECT":
            layout.operator(
                "coa_tools2.convert_deprecated_data",
                text="Update Data",
            ...
            hide_select_icon = (
                "RESTRICT_SELECT_ON" if item.hide_select else "RESTRICT_SELECT_OFF"
            )
            favorite_icon = "SOLO_ON" if item.favorite else "SOLO_OFF"
            show_children_icon = (
                "DISCLOSURE_TRI_RIGHT"
                if obj != None and not obj.coa_tools2.show_children
            ...
            if item.entry_type in ["SPRITE", "OBJECT"]:
                ...
                if item.object_type == "ARMATURE":
                    row_left.prop(
                        ...
                        text="",
                        emboss=False,
                        icon=show_children_icon,
                    )

                op = row_left.operator(
                    "coa_tools2.select_child", text=item.name, emboss=False
                )
                op.mode = "object"
                ...
                if obj != sprite_object:
                    if (
                        ...
                        row_right.prop(
                            item, "favorite", text="", icon=favorite_icon, emboss=False
                        )
                        row_right.prop(
                            item, "hide", text="", icon=hide_icon, emboss=False
                        )
                        row_right.prop(
                            item,
                            "hide_select",
                            text="",
                            icon=hide_select_icon,
                            emboss=False,
                        )
                    else:
                        if obj.type == "MESH":
                            ...
                            op = row_right.operator(
                                "coa_tools2.change_z_ordering", text="", icon="TRIA_UP"
                            )
                            op.index = item.index
                            op.direction = "DOWN"
                            op.active_sprite = item.display_name
                ...
                slot = obj.coa_tools2.slot[item.slot_index]
                if slot.active:
                    row_left.prop(
                        slot,
                        "active",
                ...
                op = row_right.operator(
                    "coa_tools2.move_slot_item", icon="TRIA_DOWN", emboss=False, text=""
                )
                op.idx = item.slot_index
                op.ob_name = item.name
                op.mode = "DOWN"
                op = row_right.operator(
                    "coa_tools2.move_slot_item", icon="TRIA_UP", emboss=False, text=""
                )
                op.idx = item.slot_index
                op.ob_name = item.name
                op.mode = "UP"
                ...
                row_right.prop(
                    item, "hide_select", text="", icon=hide_select_icon, emboss=False
                )
                ...
                if obj.type == "ARMATURE":
                    ...
                    # row_left.label(text=item.display_name)
                    op = row_left.operator(
                        "coa_tools2.select_child", text=item.display_name, emboss=False
                    )
                    op.mode = "bone"
                    op.outliner_index = item.index
                    op.ob_name = item.name
                    ...
                    if (
                        len(bone.children) > 0
                        and context.scene.coa_tools2.outliner_filter_names == ""
                    ):
...
def create_outliner_items(dummy):
    ...
    outliner_index = context.scene.coa_tools2.outliner_index
    ...
    for sprite_object in scene.objects:
        if "sprite_object" in sprite_object.coa_tools2:
    ...
    for j, obj in enumerate(items):
        if "sprite_object" in obj.coa_tools2:
            sprite_object = obj
        if (
            substring_found(scene.coa_tools2.outliner_filter_names, obj.name)
            or sprite_object == obj
        ) and (
            not scene.coa_tools2.outliner_favorites
            or (scene.coa_tools2.outliner_favorites and obj.coa_tools2.favorite)
            or sprite_object == obj
        ):
            if "sprite_object" in obj.coa_tools2 and obj.type == "EMPTY":
                ...
                "sprite_object" not in obj.coa_tools2
...
def recursive_bone_iteration(
    scene, outliner, sprite_object, obj, parent_item, bone, hierarchy_level
):
    global i

    search_active = scene.coa_tools2.outliner_filter_names != ""
    if (
        scene.coa_tools2.outliner_favorites and bone.coa_tools2.favorite
